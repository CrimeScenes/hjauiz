        ...
        if BodyPart then
            ...
            if getgenv().Silent.Startup.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if getgenv().Silent.Startup.mode == "target" then
                    ...
                    -- Check for AutoPrediction flag
                    local velocity
                    if getgenv().CamLock.Normal.AutoPrediction then
                        velocity = GetVelocity(TargetPlayer, BodyPart.Name)
                    else
                        velocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                    end
                ...
                elseif getgenv().Silent.Startup.mode == "normal" then
                    ...
                    if targetToShoot and targetToShoot.Character then
                        ...
                        if targetBodyPart then
                            ...
                            local velocity
                            if getgenv().CamLock.Normal.AutoPrediction then
                                velocity = GetVelocity(targetToShoot, targetBodyPart.Name)
                            else
                                velocity = targetToShoot.Character.HumanoidRootPart.Velocity
                            end
...
-- // Get Velocity Function with Auto Prediction (Continuous Ping Check) // --
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        -- RunService Heartbeat to continuously check the ping and update prediction
        game:GetService("RunService").Heartbeat:Connect(function()
            local ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]  -- Get the current ping

            -- Check if AutoPrediction is enabled
            if getgenv().Silent.Startup.AutoPrediction then
                -- Loop through the PlayerPredictions table and adjust the prediction based on the ping range
                for _, pred in ipairs(getgenv().PlayerPredictions) do
                    if ping >= pred.PingRange[1] and ping < pred.PingRange[2] then
                        getgenv().Silent.Startup.Prediction = pred.PredictionFactor
                        break  -- Exit the loop once the correct prediction is found
                    end
                end
            else
                -- Use normal prediction if AutoPrediction is not enabled
                getgenv().Silent.Startup.Prediction = OldPredictionY
            end
        end)

        -- Resolver-based prediction adjustments
    ...
    return Vector3.new(0, 0, 0)  -- Return zero if player or part is invalid
end
